...
CHECKS = [
    ...
    ('paia forum (no pipes)', by_url('paia.com/talk/viewtopic', 't=135'),
        lambda o: ' | ' not in o, 'forum must use normal extractor'),
    ('UniProt no <p> in piped row', by_url('uniprot.org/uniprot/P45622'),
        lambda o: not any('<p>' in ln for ln in o.split('\n') if ' | ' in ln),
        'display:none tooltip must not leak into the table'),
    ('UniProt keeps Active site', by_url('uniprot.org/uniprot/P45622'),
        lambda o: 'Active site' in o, 'cell tail text must survive display:none strip'),
