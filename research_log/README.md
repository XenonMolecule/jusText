...
- [0074 — content in `<script>` blobs](0074-script-content-negative.md) — **NEGATIVE**: iheart bio lives in a bespoke `BOOT={…}` JS app-state blob, not JSON-LD; only ~2 dev docs truly script-trapped, each different structure; JSON-LD articleBody helps 0 failing docs; no general lever — prune/gold-ceiling (needs JS execution)
- [0075 — needs-JS detector](0075-needs-js-detector.md) — **new API** `needs_javascript_render(html, paragraphs)`: flags large+script-heavy pages that extracted <300 chars (client-side SPAs) to route to a headless browser; dev 3/1000 + dev2 2/1000, 100% precision (iheart/countryliving/biography.com)
- [0076 — concatenated documents](0076-concatenated-documents.md) — **self-correcting re-merge**: WARC pages with multiple `<html>` (redirect stub + real page) make lxml stop at the first `</html>` (geneageek 0.02); extract both default and merged-bodies, keep whichever yields more content (can't regress); dev2 +0.0015, dev flat
- [0077 — JForum (coderanch)](0077-jforum.md) — **forum role-transform**: JForum authors `.authorName`/bodies `td.postbody` paired by order; coderanch/346730 (dev2) 0.376→0.970; dev2 +0.0006, dev flat
- [0078 — dev2 trend analysis](0078-dev2-trend-analysis.md) — **survey**: under-extraction frontier = walls (forum quotes), data mismatches (SPA/pagination), one-offs; over-extraction (103 docs) split between gold errors (tiny golds) and genuine chrome-leak; clean wins were structural (0076 multi-doc, 0077 JForum)
- [0079 — forum broadening](0079-forum-broadening-negative.md) — **NEGATIVE**: phpBB keep-quotes (+0.247 sum but 39 losses) and phpBB generalization (train −12.5) both regress; tight engine-specific handlers only; reverted
- [0080 — phpBB Post-as-author](0080-phpbb-post-icon-author.md) — **bug fix**: newer phpBB skins put a post-icon link ("Post") before the author link → every post titled "Post" (dhammawheel); skip icon/label links, take real author or "by NAME »" fallback; 15 wins 0 losses
- [0081 — re-merge single-body multi-docs](0081-remerge-single-body.md) — **win**: extends 0076; relax the merge guard from ≥2 `<body>` to ≥1 (a concatenated stub with a malformed `<body>` left the real doc's content past the first `</html>` where lxml stops); self-correcting so dev stays flat; cams.com 0.00→0.91, dev2 +0.0010 F1
- [0082 — DSpace item metadata](0082-dspace-metadata.md) — **win**: repository item-view (`table.itemDisplayTable`) split-cell skins drop the link-dense author value, leaving a bare "Authors" label (hub.hku.hk); rebuild Title→heading / authors `; `-joined / `Label: value`, applied ONLY when a bare metadata label is present so the 4 already-good DSpace skins are untouched; hku 0.87→1.00, dev/train flat
- [0083 — spacing at inline/block boundaries](0083-spacing-inline-boundaries.md) — **win**: words glued where `<img>` sits between words (`Enhancing<img>Education`) or adjacent `<address>` blocks touch (`TreasurerJulia`); add `img`→separator (space) and `address`→line-break; pajcisenate 0.93→0.96, dev2 flat, dev +0.0001 (some source-level glue is unrecoverable)
- [0084 — drop dangling UI labels](0084-orphan-ui-labels.md) — **quality win**: a lone "By"/"Share"/"Read More"/"Full Story" paragraph (dead JS Twitter widget, read-more button) dropped; excludes "Author"/"Comments" (real content in some gold); slysa "By/By" gone, dev2/dev flat
- [0085 — Movable Type comment attribution](0085-movabletype-comments.md) — **win**: MT blog comments (`div.comment-content` + sibling `p.comment-footer` "Posted by: NAME | DATE") were unattributed; pair body+footer, reuse `prepend_comment_authors`; also skip the injected "**Comments**" when the page already keeps its own Comments header; sauer-thompson 0.89→0.92, dev2 +0.0001, dev flat
- [0086 — MetaFilter engine handler](0086-metafilter.md) — **win**: Ask MetaFilter/MetaFilter answers (`div.comments` + trailing `span.smallcopy` "posted by X at TIME on DATE [N favorites]") → commenter-to-front `**user** (TIME, DATE) [N favorites]:` with question + `---` separator; favorites kept per user; all 5 docs +0.06-0.09, dev2/dev flat
- [0087 — CONTENTdm OCR text](0087-contentdm.md) — **win**: digital-library items render client-side; OCR full text lives in `__INITIAL_STATE__.item.item.text` not the DOM (F1=0.00). Parse the JSON blob, emit the text, gated on the `collectionAlias` key + text≥400 (fires on 0 of 203 other state-blob SPAs); westlake 0.00→0.99, uidaho 0.00→0.19 (gold-truncated), dev3 +0.0006, dev2/dev flat
- [0088 — premature `</html>` close](0088-premature-html-close.md) — **win**: an empty `<body></body></html>` closes early and the article follows as loose markup → lxml drops it all (F1=0.00); extend the self-correcting remerge to strip the premature close; thursdayreview 0.00→0.99, dev3 +0.0004, dev2/dev flat
- [0089 — StackExchange misfire on AnsPress](0089-anspress-stackexchange-misfire.md) — **win**: the SE handler fired on the AnsPress WordPress Q&A plugin (shares `id=question`/`itemprop=text`) and emitted empty markers, burying the answer; require a real `.post-text` body or fall through; housingforseniors 0.07→0.98, 32 real SE docs unaffected, dev2/dev flat
- [0090 — Ultimate Guitar review](0090-ultimate-guitar.md) — **win**: review body is HTML in `window.UGAPP.store.page.data.content` (not DOM); parse it through `ParagraphMaker` (block-aware, drops `<script>`/`<style>` — never content code); ultimate-guitar 0.00→0.91, dev2/dev flat
- [0091 — content-in-links rescue](0091-content-in-links-rescue.md) — **win** (user idea): a page whose whole content is wrapped in `<a>` (course catalogue) is killed by link-density; when normal extraction emits <300 chars, re-extract with long links unwrapped and keep if more — self-correcting, fires only on near-empty docs so working pages are untouched; hqnetwork 0.00→1.00, dev2 flat (0 fires)
- [0092 — Hearst flipbook slideshow](0092-hearst-flipbook.md) — **win**: slideshow tips live in a JS `FBModel.slides` array (Country Living / Hearst network), not the DOM; parse title+description per slide and append the missing ones post-classification (keeps the intro); countryliving 0.21→0.95, dev +0.0004 (generalized to an unseen dev doc), dev2 flat
- [0093 — JSON-LD abstract rescue](0093-jsonld-abstract-rescue.md) — **win**: a page that badly under-extracted (<1000 chars) but carries a long JSON-LD `description`/`abstract` (journal/article page whose body was dropped) gets the abstract appended; gated on under-extraction so well-extracted pages (blurb-only description) are untouched; bmcinfectdis 0.07→0.94 (medical content), dev2/dev flat
- [0094 — vBulletin threaded mode](0094-vbulletin-threaded.md) — **win**: in `?mode=threaded` posts hide in a JS `pd[postid]` preview array (load on click, so Playwright wont help); parse pd[], de-escape, emit `user: body` with quotes stripped; generalizes to vBulletin threaded+no-postbit skins (webdeveloper 0.18→0.95, wakeworld linear 0.93→0.995, sencha, littleriveroutfitters); + OP-username, quote-tail, and post_message-skin fixes
- [0095 — WordPress comment-author class](0095-wordpress-comment-author-class.md) — **win**: comment author read only from `.fn`/`.comment-author-link`/`cite`; a Blogger-on-WP theme uses `.comment-author` → comments unattributed; broaden the selector; everybodylikessandwiches 0.55→0.58 (**Comments** + 26 markers), dev2/dev/dev3 flat
- [0096 — comment byline dedupe](0096-comment-byline-dedupe.md) — **fix**: some themes keep the raw `.comment-meta` byline AND get the injected `*author* (date):` marker → double byline; drop the raw one; everybodylikessandwiches 0.58→0.59, dev2/dev/dev3 flat
