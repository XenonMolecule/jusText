...
- [0094 — vBulletin threaded mode](0094-vbulletin-threaded.md) — **win**: in `?mode=threaded` posts hide in a JS `pd[postid]` preview array (load on click, so Playwright wont help); parse pd[], de-escape, emit `user: body` with quotes stripped; generalizes to 4 vBulletin forums (webdeveloper 0.18→0.95, sencha, littleriveroutfitters, wakeworld), dev +0.0010
